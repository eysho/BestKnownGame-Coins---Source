...
#if defined(NDEBUG)
# error "Nichts cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("987b2656dcfc4de62e2eef94e8675904a50bc619c6e614489ac5cbfcf2391e0d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nichts: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nichts Signed Message:\n";
...
{
    // Nichts: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Nichts: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Nichts: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Nichts: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nichts: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nichts: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Nichts: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("f5ae71e26c74beacc88382716aced69cddf3dffff24f384e1808905e0188f68f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("36a9e41063f3e71466299d0ed9e8193c1c802a88b286016fa4a4d0c3bc384a5c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Nichts: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NichtsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NichtsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NichtsMiner(CWallet *pwallet)
{
    printf("NichtsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nichts-miner");
    ...
    try { loop {
        ...
        printf("Running NichtsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NichtsMiner, pwallet));
}
